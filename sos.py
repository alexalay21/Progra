...
class gramatica:
    ...
    def buildNFA(self):
        ...
        for char in self.regex:
            if char in self.alphabet:
                ...
                if previous in self.operators:
                    raise BaseException("Error" % (char, previous))
                while(1):
                    if len(self.stack) == 0:
                        raise BaseException("Error" % char)
                    o = self.stack.pop()
                ...
                raise BaseException("Simbolo no permitido" % char)
            previous = char
        ...
        if len(self.automata) > 1:
            print self.automata
            raise BaseException("El parseo no se realizó")
    ...
    def processOperator(self, operator):
        if len(self.automata) == 0:
            raise BaseException("Error, la cola está vacía" % operator)
        if operator == self.star:
            a = self.automata.pop()
            self.automata.append(BuildAutomata.starstruct(a))
        elif operator in self.operators:
            if len(self.automata) < 2:
                raise BaseException("Error, operandos inadecuados" % operator)
            a = self.automata.pop()
            b = self.automata.pop()
